	...
	onMount(async () => {
		if ($user?.role !== 'admin') {
			if ($page.url.pathname.includes('/models') && !$user?.permissions?.workspace?.models) {
				...
				$page.url.pathname.includes('/personas') &&
				...
				<div class="">
					...
					>
						{#if $user?.role === 'admin' || $user?.permissions?.workspace?.models}
							<a
								draggable="false"
								aria-current={$page.url.pathname.includes('/workspace/personas') ? 'page' : null}
								class="min-w-fit p-1.5 {$page.url.pathname.includes('/workspace/personas')
									? ''
									: 'text-gray-300 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white'} transition select-none"
								href="/workspace/personas">{$i18n.t('Personas')}</a
							>
