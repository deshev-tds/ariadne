...
export const getWebSearchSourceRegistry = async (token: string) => {
	let error = null;

	const res = await fetch(`${RETRIEVAL_API_BASE_URL}/web/search/planner/source-registry`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const updateWebSearchSourceRegistry = async (token: string, registry: object) => {
	let error = null;
