	...
	<div class=" space-y-3 overflow-y-scroll max-h-[28rem] md:max-h-full">
		...
		{:else}
			<div class="text-xs text-gray-500 dark:text-gray-400">
				{$WEBUI_NAME} began as a fork of
				<a class="underline" href="https://github.com/open-webui/open-webui" target="_blank"
					>Open WebUI</a
				>
				and keeps selected upstream compatibility while focusing on local-first workflows.
			</div>
		{/if}
		...
		<div>
			<pre
				class="text-xs text-gray-400 dark:text-gray-500">Portions derived from <a
		...
		<div class="mt-2 text-xs text-gray-400 dark:text-gray-500">
			{$i18n.t('Original upstream created by')}
			<a
