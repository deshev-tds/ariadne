...
app.state.config.ENABLE_TASK_MODEL_WEB_SEARCH_PLANNER = (
    ENABLE_TASK_MODEL_WEB_SEARCH_PLANNER
)
...
app.state.config.WEB_EVIDENCE_RETRIEVAL_MODE = WEB_EVIDENCE_RETRIEVAL_MODE
app.state.config.WEB_EVIDENCE_CONTEXT_MODE = WEB_EVIDENCE_CONTEXT_MODE
app.state.config.ENABLE_WEB_EVIDENCE_CONCEPT_ALIGNMENT = (
    ENABLE_WEB_EVIDENCE_CONCEPT_ALIGNMENT
)
...
async def chat_completion(
    ...
    async def process_chat(request, form_data, user, metadata, model):
        try:
            ...
            direct_response = metadata.get("direct_response")
            ...
            while True:
                try:
                    ...
                    commit_state = metadata.get("deep_research_commit_state")
                    if metadata.get("direct_response") is not None and commit_state in {
                        "committed_success",
                        "committed_failure",
                    }:
            ...
            if metadata.get("direct_response") is not None and metadata.get(
                "deep_research_commit_state"
            ) in {"committed_success", "committed_failure"}:
                raise
            try:
                event_emitter = get_event_emitter(metadata)
