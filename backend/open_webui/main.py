...
app.state.config.CONTEXT_MAINTENANCE_SAFETY_RESERVE_TOKENS = (
    CONTEXT_MAINTENANCE_SAFETY_RESERVE_TOKENS
)
app.state.config.CONTEXT_MAINTENANCE_RAG_RESERVE_TOKENS = (
    CONTEXT_MAINTENANCE_RAG_RESERVE_TOKENS
)
app.state.config.CONTEXT_MAINTENANCE_SOFT_MARGIN_TOKENS = (
    CONTEXT_MAINTENANCE_SOFT_MARGIN_TOKENS
)
app.state.config.CONTEXT_MAINTENANCE_ANCHOR_BUDGET_TOKENS = (
    CONTEXT_MAINTENANCE_ANCHOR_BUDGET_TOKENS
)
...
app.state.config.ENABLE_CORPUS_EVIDENCE_RERANKING = ENABLE_CORPUS_EVIDENCE_RERANKING
app.state.config.CORPUS_EVIDENCE_RERANKING_MODEL = CORPUS_EVIDENCE_RERANKING_MODEL
...
app.state.config.WEB_SEARCH_LOCAL_FIRST = WEB_SEARCH_LOCAL_FIRST
app.state.config.WEB_SEARCH_LOCAL_MIN_PRIMARY_HITS = WEB_SEARCH_LOCAL_MIN_PRIMARY_HITS
app.state.config.WEB_SEARCH_BRAVE_FALLBACK = WEB_SEARCH_BRAVE_FALLBACK
app.state.config.WEB_SEARCH_BRAVE_FALLBACK_MAX_QUERIES = (
    WEB_SEARCH_BRAVE_FALLBACK_MAX_QUERIES
)
app.state.config.WEB_SEARCH_BRAVE_MIN_INTERVAL_MS = WEB_SEARCH_BRAVE_MIN_INTERVAL_MS
app.state.config.ENABLE_WEB_SEARCH_PLANNER = ENABLE_WEB_SEARCH_PLANNER
app.state.config.WEB_SEARCH_PLANNER_MIN_TOTAL_QUERIES = (
    WEB_SEARCH_PLANNER_MIN_TOTAL_QUERIES
)
app.state.config.WEB_SEARCH_PLANNER_MAX_TOTAL_QUERIES = (
    WEB_SEARCH_PLANNER_MAX_TOTAL_QUERIES
)
app.state.config.WEB_SEARCH_PLANNER_MAX_TARGETED_DOMAINS_PER_WAVE = (
    WEB_SEARCH_PLANNER_MAX_TARGETED_DOMAINS_PER_WAVE
)
app.state.config.WEB_SEARCH_PLANNER_PRIMARY_STOP_SCORE = (
    WEB_SEARCH_PLANNER_PRIMARY_STOP_SCORE
)
app.state.config.WEB_SEARCH_PLANNER_PRIMARY_STOP_TRUSTED_DOMAINS = (
    WEB_SEARCH_PLANNER_PRIMARY_STOP_TRUSTED_DOMAINS
)
app.state.config.WEB_SEARCH_PLANNER_PLATEAU_FLOOR_SCORE = (
    WEB_SEARCH_PLANNER_PLATEAU_FLOOR_SCORE
)
app.state.config.WEB_SEARCH_PLANNER_PLATEAU_DELTA = WEB_SEARCH_PLANNER_PLATEAU_DELTA
app.state.config.WEB_SEARCH_PLANNER_PLATEAU_STREAK = WEB_SEARCH_PLANNER_PLATEAU_STREAK
app.state.config.WEB_SEARCH_PLANNER_MODE = WEB_SEARCH_PLANNER_MODE
app.state.config.WEB_SEARCH_PLANNER_REWRITER_MAX_QUERIES = (
    WEB_SEARCH_PLANNER_REWRITER_MAX_QUERIES
)
app.state.config.WEB_SEARCH_PLANNER_REWRITER_TIMEOUT_MS = (
    WEB_SEARCH_PLANNER_REWRITER_TIMEOUT_MS
)
app.state.config.WEB_SEARCH_PLANNER_REWRITER_MAX_REPAIR_ATTEMPTS = (
    WEB_SEARCH_PLANNER_REWRITER_MAX_REPAIR_ATTEMPTS
)
app.state.config.WEB_SEARCH_PLANNER_REWRITER_MAX_COMPLETION_TOKENS = (
    WEB_SEARCH_PLANNER_REWRITER_MAX_COMPLETION_TOKENS
)
app.state.config.WEB_SEARCH_PLANNER_REWRITER_TEMPERATURE = (
    WEB_SEARCH_PLANNER_REWRITER_TEMPERATURE
)
app.state.config.WEB_SEARCH_PLANNER_ENABLE_INTENT_COVERAGE_GUARD = (
    WEB_SEARCH_PLANNER_ENABLE_INTENT_COVERAGE_GUARD
)
...
app.state.config.NEWS_ARTICLE_MODEL_TIMEOUT_SECONDS = NEWS_ARTICLE_MODEL_TIMEOUT_SECONDS
app.state.config.NEWS_BRIEF_MODEL_TIMEOUT_SECONDS = NEWS_BRIEF_MODEL_TIMEOUT_SECONDS
app.state.config.NEWS_BRIEF_TARGET_ITEM_COUNT = NEWS_BRIEF_TARGET_ITEM_COUNT
...
app.state.config.WEB_SEARCH_EVIDENCE_JUDGE_EVERY_CHUNKS = (
    WEB_SEARCH_EVIDENCE_JUDGE_EVERY_CHUNKS
)
app.state.config.WEB_SEARCH_EVIDENCE_JUDGE_MIN_CHUNKS = (
    WEB_SEARCH_EVIDENCE_JUDGE_MIN_CHUNKS
)
app.state.config.WEB_SEARCH_EVIDENCE_JUDGE_CONFIDENCE = (
    WEB_SEARCH_EVIDENCE_JUDGE_CONFIDENCE
)
app.state.config.WEB_SEARCH_EVIDENCE_JUDGE_TIMEOUT_MS = (
    WEB_SEARCH_EVIDENCE_JUDGE_TIMEOUT_MS
)
app.state.config.WEB_SEARCH_EVIDENCE_JUDGE_MAX_COMPLETION_TOKENS = (
    WEB_SEARCH_EVIDENCE_JUDGE_MAX_COMPLETION_TOKENS
)
app.state.config.WEB_SEARCH_EVIDENCE_JUDGE_MAX_INPUT_CHARS = (
    WEB_SEARCH_EVIDENCE_JUDGE_MAX_INPUT_CHARS
)
app.state.config.WEB_SEARCH_STRONG_FETCH_RERANK_CHUNK_SIZE = (
    WEB_SEARCH_STRONG_FETCH_RERANK_CHUNK_SIZE
)
app.state.config.WEB_SEARCH_STRONG_FETCH_RERANK_CHUNK_OVERLAP = (
    WEB_SEARCH_STRONG_FETCH_RERANK_CHUNK_OVERLAP
)
app.state.config.WEB_SEARCH_STRONG_FETCH_RERANK_EXCERPT_CHARS = (
    WEB_SEARCH_STRONG_FETCH_RERANK_EXCERPT_CHARS
)
...
app.state.config.PLAYWRIGHT_REMOVE_SELECTORS = PLAYWRIGHT_REMOVE_SELECTORS
...
async def generate_messages(
    ...
    All models configured in Ariadne are accessible via this endpoint.
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for Ariadne.
...
async def get_manifest_json():
    if app.state.EXTERNAL_PWA_MANIFEST_URL:
        ...
        return {
            ...
            "description": f"{app.state.WEBUI_NAME} is a local-first AI workspace for long-context chat, personas, recall, and evidence-oriented workflows.",
            "start_url": "/",
            "display": "standalone",
