...
# device type embedding models - "cpu" (default), "cuda" (CUDA or ROCm/HIP), or
# "mps" (Apple Silicon). Prefer the actual runtime capability over a docker-only flag.
...
WEBUI_NAME = os.environ.get("WEBUI_NAME", "Ariadne")
...
TERMINAL_TOOL_RESULT_INLINE_MAX_BYTES = os.environ.get(
    "TERMINAL_TOOL_RESULT_INLINE_MAX_BYTES", "16384"
)
if TERMINAL_TOOL_RESULT_INLINE_MAX_BYTES == "":
    TERMINAL_TOOL_RESULT_INLINE_MAX_BYTES = 16384
else:
    try:
        TERMINAL_TOOL_RESULT_INLINE_MAX_BYTES = int(
            TERMINAL_TOOL_RESULT_INLINE_MAX_BYTES
        )
    except Exception:
        TERMINAL_TOOL_RESULT_INLINE_MAX_BYTES = 16384

TERMINAL_TOOL_RESULT_PREVIEW_CHARS = os.environ.get(
    "TERMINAL_TOOL_RESULT_PREVIEW_CHARS", "2000"
)
if TERMINAL_TOOL_RESULT_PREVIEW_CHARS == "":
    TERMINAL_TOOL_RESULT_PREVIEW_CHARS = 2000
else:
    try:
        TERMINAL_TOOL_RESULT_PREVIEW_CHARS = int(TERMINAL_TOOL_RESULT_PREVIEW_CHARS)
    except Exception:
        TERMINAL_TOOL_RESULT_PREVIEW_CHARS = 2000
...
RAG_WEB_FULL_CONTEXT_ONCE = (
    os.environ.get("RAG_WEB_FULL_CONTEXT_ONCE", "True").lower() == "true"
)
