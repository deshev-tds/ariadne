...
def _registry_cache_key(root: Path, taxonomy_path: Path, review_path: Path) -> tuple[str, float, float]:
    try:
    ...
    with _REGISTRY_LOCK:
        ...
        for item in taxonomy.get("books") or []:
            ...
            books_by_id[book_id] = OffsecBook(
                ...
                use_when=tuple(
                    _normalize_text(tag) for tag in item.get("use_when") or []
                ),
                strong_for=tuple(
                    _normalize_text(tag) for tag in item.get("strong_for") or []
                ),
                weak_for=tuple(
                    _normalize_text(tag) for tag in item.get("weak_for") or []
                ),
...
def _route_consultation(
    ...
    *,
    ...
    query = " ".join(
        value for value in [named_entity, objective, current_findings, current_hypothesis] if value
    )
...
def _books_for_route(
    ...
    if route == "domain":
        return [
            book
            for book in registry.books_by_id.values()
            if book.primary_domain == matched_id
        ]
...
def _should_suggest_docs(
    *,
    ...
    combined = " ".join(
        value for value in [objective, current_findings, current_hypothesis, named_entity] if value
    )
