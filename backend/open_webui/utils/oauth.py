            ...
            async with session.post(
                ...
                if response.status == 401:
                    ...
                    if match:
                        ...
                        log.debug(
                            f"Found resource_metadata URL: {resource_metadata_urls[0]}"
                        )
                    else:
                        ...
                        log.debug(
                            "No resource_metadata in header, trying well-known URIs: "
                            f"{resource_metadata_urls}"
                        )
                    ...
                    for resource_metadata_url in resource_metadata_urls:
                        try:
                            ...
                            log.debug(
                                "Failed to fetch resource metadata from "
                                f"{resource_metadata_url}: {e}"
                            )
                            continue
    except Exception as e:
    ...
    if parsed.path and parsed.path != "/":
        ...
        urls.extend(
            [
                ...
                urllib.parse.urljoin(
                    base_url, f"/.well-known/openid-configuration{path}"
                ),
                urllib.parse.urljoin(
                    base_url, f"{path}/.well-known/openid-configuration"
                ),
            ]
        )
    ...
    def ensure_client_from_config(self, client_id):
        ...
        for connection in connections or []:
            if connection.get("type", "openapi") != "mcp":
                continue
            if connection.get("auth_type", "none") not in (
                "oauth_2.1",
                "oauth_2.1_static",
            ):
                continue
