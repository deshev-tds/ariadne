...
if [[ -n "$LLAMA_REF" ]]; then
  ...
  log "  warning:  LLAMA_REF is empty; floating branch builds may reuse a cached clone layer"
  log "            Prefer --ref for provenance, or --no-cache when intentionally tracking a branch"
fi
